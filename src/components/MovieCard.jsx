...
function MovieCard({ movie }) {
    ...
    const title = movie.title || movie.title_english || "Untitled";
    const imageUrl = movie.images?.webp?.large_image_url || movie.images?.jpg?.large_image_url || movie.cover_image || "/fallback.jpg";
    const publicationYear = movie.published?.prop?.from?.year || movie.year || (movie.published?.from ? new Date(movie.published.from).getFullYear() : "N/A");
    const score = movie.score || movie.rating || "N/A";
    const genres = movie.genres?.slice(0, 2).map(g => g.name).join(" • ") || "Manga";
